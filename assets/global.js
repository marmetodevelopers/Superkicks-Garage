...
//code for text prior to upsell on pdp
document.addEventListener('DOMContentLoaded', function () {
  const hasvalue = document.querySelector(".product-off-text_line")?.getAttribute("data-one-line");
  const valueText = document.querySelector(".product-off-text_line")?.getAttribute("data-one-line-text");
  if(hasvalue){
    let hasRun = false;
    
    const checkAndRun = () => {
      const targetElement = document.querySelector('lb-upsell-product-page-addon');
    
      if (targetElement && !hasRun) {
        const htmlContent = '<div class="before-description-text">' + valueText + '</div>'; 
